(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    fetchAndDisplayvisaCategorys();
  });

  // Function to fetch and display visa types
  const fetchAndDisplayvisaCategorys = async () => {
    try {
      const response = await fetch(
        `api/v1/loadData.php?load=visa_categories&aid=${adminId}`
      );
  ...
  const rendervisaCategorys = (data) => {
  ...
  const fetchUpdatedData = (postId = null) => {
    $.ajax({
      url: "forms/edit/visa_category.php",
      method: "GET",
      data: { post_id: postId },
      success: (response) => {
  ...
  const handleBan = async (id) => {
    ...
    if (confirmDeletion.isConfirmed) {
      try {
        const response = await fetch(
          `api/v1/activate.php?id=${id}&t=visa_categories&action=ban`,
          {
            method: "POST",
          }
        );
  ...
  const handleUnban = async (id) => {
    ...
    if (confirmDeletion.isConfirmed) {
      try {
        const response = await fetch(
          `api/v1/activate.php?id=${id}&t=visa_categories&action=unban`,
          {
            method: "POST",
          }
        );
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDeletion.isConfirmed) {
      try {
        const response = await fetch(
          `api/v1/delete.php?id=${id}&t=visa_categories`,
          {
            method: "DELETE",
          }
        );
  ...
  const attachFormSubmissionListener = () => {
    const form = document.getElementById("visaCategoryForm");
    ...
    if (form) {
      form.addEventListener("submit", async (event) => {
        ...
        const visaCategory = document
          .getElementById("visaCategory")
          ?.value.trim();
        const adminID = document
          .querySelector('input[name="adminID"]')
          ?.value.trim();

        if (!visaCategory || !adminID) {
          Swal.fire({
            title: "Error!",
        ...
        try {
          const response = await fetch("api/v1/add_visa_category.php", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
            body: JSON.stringify({ visaCategory, adminID }),
          });
  ...
  const attachEditFormSubmissionListener = () => {
    const editForm = document.getElementById("editvisaCategoryForm");
    ...
    if (editForm) {
      editForm.addEventListener("submit", async (event) => {
        ...
        try {
          const response = await fetch("api/v1/edit_visa_category.php", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
